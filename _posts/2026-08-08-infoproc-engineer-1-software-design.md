...
tags: [정보처리기사, 소프트웨어공학, UML, 디자인패턴, GoF, 결합도, 응집도, SOLID, 애자일, 스크럼, XP, DFD, 미들웨어, EAI, 생명주기모형, 나선형모델, CBD, 코드설계]
toc: true
mermaid: true
---
...
> 🔥 다섯 유형의 이름-정의 매칭 문제로 자주 출제된다.
{: .prompt-tip }

| 구분 | 설명 |
|---|---|
| **CLI** (Command Line Interface) | 명령과 출력이 텍스트로 이뤄지는 인터페이스 |
| **GUI** (Graphical User Interface) | 아이콘·메뉴를 마우스로 선택해 조작하는 그래픽 환경 |
| **NUI** (Natural User Interface) | 사용자의 말이나 행동으로 기기를 조작 |
| **VUI** (Voice User Interface) | 사람의 음성으로 기기를 조작 |
| **OUI** (Organic User Interface) | 모든 사물과 사용자 간 상호작용을 위한 인터페이스. IoT·VR·AR·MR과 함께 대두됨 |
...
```mermaid
flowchart TD
    GoF[GoF 디자인 패턴 23종] --> C["생성 패턴 (5)<br/>객체를 만드는 방법에 관여"]
    GoF --> S["구조 패턴 (7)<br/>클래스·객체를 조합해 더 큰 구조를 구성"]
    GoF --> B["행위 패턴 (11)<br/>객체 간 책임·역할 분배와 상호작용"]
```

**생성 패턴 (Creational, 5종)** — 객체를 생성하는 절차를 캡슐화

| 패턴 | 설명 |
|---|---|
| **싱글톤 (Singleton)** | 클래스의 인스턴스를 **오직 하나만** 생성되도록 보장 |
| **팩토리 메소드 (Factory Method)** | 객체 생성을 서브클래스에게 위임 → 어떤 클래스의 인스턴스를 만들지는 서브클래스가 결정 |
| **추상 팩토리 (Abstract Factory)** | 서로 관련 있는 객체들의 **집합(제품군)**을 구체 클래스를 명시하지 않고 생성 |
| **빌더 (Builder)** | 복잡한 객체를 생성하는 절차를 단계별로 분리해, 동일한 절차로 서로 다른 표현의 객체를 생성 |
| **프로토타입 (Prototype)** | 원본 객체를 **복제(clone)**하여 새로운 객체를 생성 |

**구조 패턴 (Structural, 7종)** — 클래스·객체를 조합해 더 큰 구조를 만드는 방법

| 패턴 | 설명 |
|---|---|
| **어댑터 (Adapter)** | 호환되지 않는 인터페이스를 클라이언트가 요구하는 인터페이스로 변환 |
| **브리지 (Bridge)** | 추상화(기능)와 구현을 분리하여 각각 독립적으로 확장 가능하게 함 |
| **컴포지트 (Composite)** | 객체들을 트리 구조로 구성해 부분-전체 계층을 표현, 개별 객체와 복합 객체를 동일하게 다룸 |
| **데코레이터 (Decorator)** | 객체에 **동적으로 새로운 책임(기능)**을 추가, 상속의 대안 |
| **퍼사드 (Facade)** | 복잡한 서브시스템에 대해 단순화된 **단일 인터페이스**를 제공 |
| **플라이웨이트 (Flyweight)** | 공유 가능한 객체를 캐싱해 재사용함으로써 **메모리 사용량**을 줄임 |
| **프록시 (Proxy)** | 실제 객체에 대한 접근을 제어하는 **대리 객체**를 제공 |

**행위 패턴 (Behavioral, 11종)** — 객체 간 책임과 알고리즘, 상호작용 방식을 정의

| 패턴 | 설명 |
|---|---|
| **커맨드 (Command)** | 요청(실행할 작업)을 객체로 캡슐화해 실행을 지연하거나 큐잉·취소 가능하게 함 |
| **책임 연쇄 (Chain of Responsibility)** | 요청을 처리할 객체들을 사슬처럼 연결, 처리할 수 있는 객체가 나올 때까지 전달 |
| **인터프리터 (Interpreter)** | 언어의 문법을 클래스화하여, 특정 언어의 문장을 해석 |
| **반복자 (Iterator)** | 컬렉션의 내부 구조를 노출하지 않고 요소에 **순차적으로 접근**하는 방법을 제공 |
| **중재자 (Mediator)** | 객체들 간의 복잡한 상호작용을 **중재자 객체**로 캡슐화해 결합도를 낮춤 |
| **메멘토 (Memento)** | 객체의 상태를 저장해두었다가 **이전 상태로 복원**할 수 있게 함 |
| **옵서버 (Observer)** | 한 객체의 상태가 바뀌면 그 객체에 의존하는 다른 객체들에게 자동으로 통지하는 **일대다 의존 관계** |
| **상태 (State)** | 객체의 내부 상태에 따라 객체의 행위(동작)를 변경 |
| **전략 (Strategy)** | 알고리즘군을 각각 캡슐화하여 상호 교체 가능하게 함, 클라이언트와 알고리즘을 분리 |
| **템플릿 메소드 (Template Method)** | 알고리즘의 골격(구조)을 상위 클래스에 정의하고, 세부 단계는 서브클래스에서 구현 |
| **방문자 (Visitor)** | 객체 구조는 그대로 둔 채, 구조를 방문하며 수행할 **새로운 연산**을 별도 클래스로 추가 |

> 🔥 시험에서는 "다음 설명에 해당하는 디자인 패턴은?" 형태로 **설명 문장 → 패턴 이름 매칭**이 가장 많이 나온다. 특히 싱글톤·어댑터·옵서버·전략·팩토리 메소드는 매회 출제되는 단골.
{: .prompt-tip }
...
### 4-7. 소프트웨어 품질 요구사항 (ISO/IEC 9126)

> 🔥 5과목 정보시스템 구축관리와 겹치는 초고빈도 주제. 6대 특성과 하위 특성을 세트로 암기.
{: .prompt-tip }

| 특성 | 의미 | 하위 특성 |
|---|---|---|
| **기능성** (Functionality) | 요구된 기능을 정확히 제공하는지 | 적합성, 정확성, 상호운용성, 보안성, 준수성 |
| **신뢰성** (Reliability) | 오류 없이 일관되게 수행하는 정도 | 성숙성, 고장 허용성, 회복성 |
| **사용성** (Usability) | 이해·학습·운용이 쉬운 정도 | 이해성, 학습성, 운용성, 친밀성 |
| **효율성** (Efficiency) | 한정된 자원으로 빠르게 처리하는 정도 | 시간 효율성, 자원 효율성 |
| **유지보수성** (Maintainability) | 변경·확장이 쉬운 정도 | 분석성, 변경성, 안정성, 시험성 |
| **이식성** (Portability) | 다른 환경에서도 쉽게 적용되는 정도 | 적용성, 설치성, 대체성, 공존성 |

⚠️ ISO/IEC 9126을 개정한 것이 **ISO/IEC 25010**, 9126과 14598을 통합한 것이 **ISO/IEC 25000 (SQuaRE)**이다.
...
### 6-1-1. 자료 사전 (DD, Data Dictionary) 기호

DFD의 자료를 더 자세히 정의·기록한 것으로, "데이터의 데이터"라는 의미에서 **메타데이터**라고도 한다.
...
### 7-3. 스테레오 타입 (Stereotype)

UML의 기본 기능 외에 추가 기능을 표현하기 위해, 길러멧(Guillemet)이라 부르는 겹화살괄호(`<< >>`) 사이에 표기한다.

| 표기 | 의미 |
|---|---|
| `<<include>>` | 다른 UML 요소를 포함하는 관계 |
| `<<extend>>` | 다른 UML 요소를 확장하는 관계 |
| `<<interface>>` | 인터페이스를 정의 |
| `<<exception>>` | 예외를 정의 |
...
### 9-1. 애자일 선언문 4가지 핵심 가치

> 🔥 순서와 "~보다 ~에 더 가치를 둔다"는 문장 구조까지 그대로 출제된다.
{: .prompt-tip }

1. 프로세스와 도구보다는 **개인과 상호작용**에 더 가치를 둔다.
2. 방대한 문서보다는 **실행되는 SW**에 더 가치를 둔다.
3. 계약 협상보다는 **고객과의 협업**에 더 가치를 둔다.
4. 계획을 따르기보다는 **변화에 대한 반응**에 더 가치를 둔다.
...
### 9-3. 익스트림 프로그래밍 (eXtreme Programming, XP)
...
## 10. 소프트웨어 생명주기(SDLC) 모형

소프트웨어를 기획부터 폐기까지 어떤 순서·방식으로 개발할지를 정의한 모형. **모형 이름-특징 매칭 문제**가 매회 출제되는 핵심 주제다.

### 10-1. 폭포수 모형 (Waterfall Model)

가장 오래되고 가장 널리 쓰인 고전적 생명주기 모형. **한 단계가 끝나야만 다음 단계로 진행**하는 선형 순차적 모형이다.

```mermaid
flowchart LR
    A[타당성 검토] --> B[계획] --> C[요구 분석] --> D[설계] --> E[구현/코딩] --> F[테스트] --> G[유지보수]
```

- 단계별 정의와 산출물이 명확해 관리가 쉬움
- ⚠️ 각 단계가 끝난 뒤에야 다음 단계로 넘어가므로 **개발 중간에 요구사항 변경이 어려움**
- 순차적 접근으로, 이전 단계로 돌아가는 피드백(재작업)이 큰 비용을 유발

### 10-2. 프로토타입 모형 (Prototype Model)

요구사항이 불명확할 때, **시제품(프로토타입)**을 먼저 만들어 사용자 피드백을 받은 뒤 개발을 진행하는 모형.

```mermaid
flowchart LR
    R[요구사항 수집] --> P[프로토타입 설계·개발] --> D[사용자 시연/평가] --> R2{요구사항<br/>충족?}
    R2 -- No --> P
    R2 -- Yes --> S[실제 시스템 개발]
```

- 사용자가 최종 결과물을 미리 확인할 수 있어 요구사항 도출에 유리
- 프로토타입은 실제 시스템의 골격이 되거나, 검증 후 폐기되기도 함

### 10-3. 나선형 모형 (Spiral Model)

**폭포수 모형 + 프로토타입 모형의 장점에 위험 분석(Risk Analysis)을 추가**한 모형. 점진적으로 개발 과정을 반복하며 요구사항을 추가·보완할 수 있다.

```mermaid
flowchart TD
    A[1. 계획 수립<br/>Planning] --> B[2. 위험 분석<br/>Risk Analysis]
    B --> C[3. 개발 및 검증<br/>Engineering]
    C --> D[4. 고객 평가<br/>Customer Evaluation]
    D -->|다음 반복 Iteration| A
```

- 4단계(계획 수립 → 위험 분석 → 개발 및 검증 → 고객 평가)를 나선을 그리듯 반복하며 점진적으로 완벽한 시스템을 구축
- 대규모·고위험 프로젝트에 적합하지만, 반복 관리가 복잡하고 비용이 늘어날 수 있음

> 🔥 "위험 분석(Risk Analysis)이 추가된 모형은?" → **나선형 모형**. "한 단계가 끝나야 다음 단계로 넘어가는 모형은?" → **폭포수 모형**. 정의 문장으로 모형을 구분하는 문제가 자주 나온다.
{: .prompt-tip }
...
자료를 컴퓨터가 처리하기 쉽도록 일정한 규칙에 따라 표현하는 방법.

| 코드 | 설명 |
|---|---|
| **순차 코드 (Sequence Code)** | 일련번호를 순서대로(1, 2, 3, …) 부여하는 가장 단순한 코드 |
| **블록 코드 (Block Code)** | 공통성이 있는 항목끼리 블록으로 구분한 뒤, 블록 내에서 일련번호를 부여 |
| **10진 코드 (Decimal Code)** | 코드를 10진 단위로 세분화하여 계층적으로 구분 (예: 도서 분류의 듀이 10진 분류법) |
| **그룹 분류 코드 (Group Classification Code)** | 코드를 대분류-중분류-소분류 등 **의미 있는 그룹**으로 나누어 구성 |
| **연상 코드 (Mnemonic Code)** | 대상의 특성을 기억하기 쉬운 문자·기호로 표현 (예: TV-40 → 40인치 TV) |
| **표의 숫자 코드 (Significant Digit Code)** | 대상의 크기, 규격 등 **속성값 자체**를 코드로 사용 |
| **합성 코드 (Combined Code)** | 두 개 이상의 코드 방식을 혼합하여 구성 |
...
| 방법론 | 핵심 특징 |
|---|---|
| **구조적 방법론** | 자료 흐름도(DFD)·자료 사전 등을 이용한 **하향식** 기능 분해 중심 |
| **정보공학 방법론** | 데이터를 중심으로 시스템을 정형화된 절차에 따라 **전사적**으로 개발 |
| **객체지향 방법론** | 클래스·객체 단위로 시스템을 모델링, 재사용성·확장성이 높음 |
| **CBD (Component Based Development, 컴포넌트 기반 개발)** | 재사용 가능한 **컴포넌트(교체 가능한 소프트웨어 부품)**를 조립해 시스템을 구성 |
| **애자일 방법론** | 고객과의 소통과 짧은 반복 주기를 중심으로 변화에 유연하게 대응 |

### 12-1. CBD (컴포넌트 기반 개발) 상세

컴포넌트란 **인터페이스로 접근 가능하고, 독립적인 기능을 수행하며, 교체가 가능한 소프트웨어 부품**을 말한다.

```mermaid
flowchart LR
    subgraph CD["CD 단계 (컴포넌트 개발)"]
        D1[도메인 분석] --> D2[컴포넌트 대상 업무 선별] --> D3[컴포넌트 개발]
    end
    subgraph CBD["CBD 단계 (컴포넌트 기반 개발)"]
        E1[요구 분석] --> E2[컴포넌트 기반 설계] --> E3[저장소에서 컴포넌트 검색·조립]
    end
    CD --> CBD
```

- **재사용성**이 우수하고 **생산성**을 높일 수 있음
- 검증된 컴포넌트를 재사용하므로 **품질 향상**과 **개발 기간 단축**에 유리
