...
> 🔥 중위(Infix)를 전위(Prefix)·후위(Postfix)로 바꾸는 문제가 매회 출제된다.
{: .prompt-tip }
...
| 표기법 | 순서 | 결과 |
|---|---|---|
| **전위 (Prefix)** | 연산자 → Left → Right | `+AB` |
| **중위 (Infix)** | Left → 연산자 → Right | `A+B` |
| **후위 (Postfix)** | Left → Right → 연산자 | `AB+` |

⚠️ Infix → Postfix/Prefix 변환은 **스택**을 이용해 처리한다는 것이 핵심 원리다.
...
### 5-8-1. V-모델 (개발 단계별 테스트 레벨)

애플리케이션 테스트를 소프트웨어 개발 단계와 짝지어 표현한 모델.

```mermaid
flowchart LR
    R[요구사항] -.대응.-> AT[인수 테스트]
    S[분석/명세] -.대응.-> ST[시스템 테스트]
    D[설계] -.대응.-> IT[통합 테스트]
    C[구현] -.대응.-> UT[단위 테스트]
```
...
| 개발 단계 | 테스트 단계 |
|---|---|
| 요구사항 (Requirements) | 인수 테스트 (Acceptance) |
| 분석 (Specification) | 시스템 테스트 (System) |
| 설계 (Design) | 통합 테스트 (Integration) |
| 구현 (Code) | 단위 테스트 (Unit) |
...
### 6-3. 순환 복잡도 (Cyclomatic Complexity)

> 🔥 `V(G) = E - N + 2` (E: 화살표 수, N: 노드 수) 공식으로 계산하는 문제가 자주 나온다.
{: .prompt-tip }

- 맥케이브(McCabe) 복잡도 메트릭이라고도 하며, 제어 흐름도의 **영역 수**와 일치함
- 프로그램의 독립적인 경로의 수를 정의하고, 모든 경로가 한 번 이상 수행되었음을 보장하기 위한 **테스트 횟수의 상한선**을 제공

### 6-4. 빅오 표기법 (Big-O Notation)

알고리즘의 **최악의 경우** 실행 시간을 표기하는 방법.

| 표기 | 의미 | 예 |
|---|---|---|
| **O(1)** | 입력값과 무관하게 일정 | 스택의 Push/Pop |
| **O(log₂n)** | 매 단계마다 대상이 절반씩 감소 | 이진 검색 |
| **O(n)** | 입력값과 1:1 비례 | for문 1회 순회 |
| **O(nlog₂n)** | n번을 log₂n만큼 수행 | 힙 정렬, 합병 정렬 |
| **O(n²)** | 입력값의 제곱만큼 수행 | 삽입·선택·버블·퀵 정렬 |
| **O(2ⁿ)** | 입력값의 지수만큼 수행 | 피보나치 수열(재귀) |

### 6-5. 소스 코드 품질 분석 도구

| 구분 | 특징 | 예 |
|---|---|---|
| **정적 분석 도구** | 코드를 실행하지 않고 코딩 스타일·표준·복잡도를 분석 | pmd, cppcheck, SonarQube, checkstyle, ccm, cobertura |
| **동적 분석 도구** | 코드를 실행해 메모리 누수·스레드 결함 등을 분석 | Avalanche, Valgrind |
...
| 항목 | 설명 |
|---|---|
| **저장소 (Repository)** | 최신 버전 파일과 변경 내역 정보가 저장된 곳 |
| **가져오기 (Import)** | 버전 관리가 없는 저장소에 처음으로 파일을 복사 |
| **체크아웃 (Check-Out)** | 저장소에서 파일(+버전 관리 파일)을 받아옴 |
| **체크인 (Check-In)** | 체크아웃한 파일의 수정을 완료하고 저장소에 새 버전으로 갱신 |
| **커밋 (Commit)** | 체크인 시 이전에 갱신된 내용이 있으면 충돌(Conflict)을 알리고 diff로 수정 후 갱신 완료 |
| **동기화 (Update)** | 저장소의 최신 버전으로 자신의 작업 공간을 동기화 |

### 8-1-1. 릴리즈 노트 (Release Note)

개발 과정에서 정리된 릴리즈 정보를 최종 사용자와 공유하기 위한 문서. **개발팀이 직접 현재 시제로** 작성해야 하며, Header, 개요, 목적, 문제 요약, 재현 항목, 수정/개선 내용, 사용자 영향도, 노트, 면책 조항, 연락처 등이 포함된다.
