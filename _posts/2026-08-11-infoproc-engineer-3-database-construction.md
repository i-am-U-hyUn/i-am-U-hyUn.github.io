...
[2과목 소프트웨어 개발](/posts/infoproc-engineer-2-software-development/)에 이어 **3과목 데이터베이스 구축**이다. 직접 정리한 초안(트랜잭션 ACID, 데이터베이스 설계 단계, 키의 종류, 정규화·반정규화, 관계대수·관계해석, 병행제어, 파티셔닝 등)을 기반으로, 시험에 자주 나오지만 초안에는 없던 트랜잭션 상태·격리 수준, 릴레이션 구성요소, 함수 종속, 데이터 사전, 무결성 제약조건, 회복 기법, 로킹 단위, 교착상태 해결법, SQL 기본 문법, 인덱스·뷰 등을 최대한 보강했다. 이후 실제 기출 경향 분석 자료를 교차 확인해 데이터 독립성, 서브쿼리, UNIQUE 제약조건, ERD 참여도 등을 추가로 보강했다.
...
### 1-4. 데이터 독립성 (Data Independence)

| 구분 | 설명 |
|---|---|
| **논리적 데이터 독립성** | 개념 스키마가 변경되어도 외부 스키마(응용 프로그램)는 영향을 받지 않음 |
| **물리적 데이터 독립성** | 내부 스키마(저장 구조)가 변경되어도 개념 스키마는 영향을 받지 않음 |

> 🔥 DBMS의 기능, 스키마의 종류와 함께 **데이터 독립성**은 실제 기출에서 매회 고정적으로 등장하는 단골 문제다.
{: .prompt-tip }
...
**참여도 (Participation Constraint)**

| 구분 | 설명 |
|---|---|
| **전체 참여 (Total Participation)** | 개체 집합의 모든 개체가 반드시 관계에 참여 |
| **부분 참여 (Partial Participation)** | 개체 집합의 일부 개체만 관계에 참여할 수도 있음 |
...
| **고유 무결성 (UNIQUE)** | 지정된 속성(열)의 값이 릴레이션 내에서 중복되지 않아야 함 |

> ⚠️ PRIMARY KEY는 NULL 불가·릴레이션당 1개만 지정 가능하지만, **UNIQUE는 NULL을 허용**하고 한 테이블에 여러 개 지정할 수 있다 — 기출에서 자주 비교 출제된다.
{: .prompt-warning }
...
### 9-1. 분산 데이터베이스 (Distributed Database)

물리적으로는 여러 컴퓨터 사이트에 나뉘어 있지만, 논리적으로는 하나의 시스템처럼 동작하는 데이터베이스.

> 🔥 4대 투명성은 5과목 정보시스템 구축관리와도 겹치는 초고빈도 주제.
{: .prompt-tip }

| 투명성 | 설명 |
|---|---|
| **위치 투명성** | 실제 저장 위치를 몰라도 논리적 명칭만으로 접근 가능 |
| **중복 투명성** | 동일 데이터가 여러 곳에 중복되어도 하나만 존재하는 것처럼 사용 |
| **병행 투명성** | 여러 트랜잭션이 동시에 실행되어도 결과에 영향 없음 |
| **장애 투명성** | 트랜잭션·DBMS·네트워크 장애에도 트랜잭션을 정확히 처리 |

- 장점: 지역 자치성, 자료 공유·점진적 확장 용이, 신뢰성·가용성 향상
- 단점: DBMS 기능·설계가 복잡, 소프트웨어 개발·처리 비용 증가
...
**서브쿼리 (Subquery)**: SELECT 문 안에 또 다른 SELECT 문을 포함하는 질의. WHERE, FROM, SELECT 절 어디에도 올 수 있음

> 🔥 서브쿼리는 JOIN·GROUP BY와 함께 **거의 매 시험에 출제되는 고빈출 SQL 유형**이다.
{: .prompt-tip }
...
| **SELF JOIN** | 같은 테이블을 자기 자신과 조인. 별칭(Alias)으로 두 테이블처럼 구분해서 사용 |

> 🔥 최근 기출은 기본 INNER/LEFT JOIN을 넘어 **SELF JOIN, RIGHT OUTER JOIN**까지 응용해서 묻는 경향이 강해지고 있다.
{: .prompt-tip }

### 10-4. 집합 연산자를 이용한 통합 질의

| 연산자 | 설명 |
|---|---|
| **UNION** | 두 SELECT 결과를 합집합으로 통합, **중복 행 제거** |
| **UNION ALL** | 합집합으로 통합하되 **중복 행도 그대로 유지** |
| **INTERSECT** | 두 결과의 공통된 행(교집합)만 반환 |
| **EXCEPT (MINUS)** | 첫 번째 결과에서 두 번째 결과를 제외한 행 반환 |

### 10-5. 트리거 (Trigger)

데이터의 삽입·갱신·삭제 이벤트가 발생할 때 관련 작업이 자동으로 수행되는 절차형 SQL. 데이터베이스에 저장되며, **트리거 구문 안에는 COMMIT·ROLLBACK 등 DCL을 사용할 수 없다.**
